...
key_down        = &41 + &80           ; A
key_up          = &5A + &80           ; Z
key_left        = &08 + &80           ; left arrow
key_right       = &15 + &80           ; right arrow
key_grid        = &47 + &80           ; G
key_cell        = &20 + &80           ; space
key_start       = &0D + &80           ; return

ENDIF

numcols = rowlen * pixels_per_byte / 2
...
.EVEN7S
FOR i, 0, numcols - 1
       EQUB 2 * (i DIV pixels_per_byte)
NEXT

ORG BASE + &300
.SEVENS
FOR i, 0, numcols - 1
       EQUB (i MOD pixels_per_byte)
NEXT
...
.BYTE1   EQUB &E0,&38,&0E,&03,&00,&00,&00,&00
.BYTE2   EQUB &00,&00,&00,&80,&E0,&38,&0E,&03
.BYTE3   EQUB &00,&00,&00,&00,&00,&00,&00,&80
...
IF _atom
.HDAT    EQUB &EE,&EE,&EE,&EE
.VDAT    EQUB &11,&11,&11,&11
ELSE        
...
.ST1
IF NOT(_atom)
         BIT &C010      ; STROBE unlatched keyboard data
ENDIF
.L1      DEC CCNT
         BNE L2
         LDA CSTAT
...
ENDIF
         CMP #key_down  ; ascii 41 = A = move cursor up
         BNE L3
         LDA VERT
         CMP #startrow  ; top limit = 6
         BEQ ST1
         LDX CSTAT
         BEQ L21
         JSR CELL
.L21     LDA VERT       ; decrement VERT by 3
         ...
         JSR CELL
.L22     JMP ST1
.L3      CMP #key_up    ; ascii 5A = Z = move cursor down
         BNE L4
         LDA VERT
         CMP #endrow    ; bottom limit = BA = 186 (6,9,12,...,183,186 gives 61 vertical positions)
         BEQ L22
         LDX CSTAT
         BEQ L31
         JSR CELL
.L31     LDA VERT
         ...
         JSR CELL
.L32     JMP ST1
.L4      CMP #key_left  ; ascii 08 = cursor left
         BNE L5
...
.L41     DEC HORZ       ; decrement HORZ by 2
         ...
         JSR CELL
.L42     JMP ST1
.L5      CMP #key_right ; ascii 15 = cursor right
         BNE L6
         LDA HORZ
         CMP #numcols - 2  ; right limit = 138 (0,2,4,...,136,138 gives 70 horizontal positions)
         BEQ L42
         LDX CSTAT
         BEQ L51
         JSR CELL
.L51     INC HORZ       ; increment HORZ by  2
         ...
         JSR CELL
.L52     JMP ST1
.L6      CMP #key_grid  ; ascii 47 = G, toggle grid
         BNE L7
         JSR GRID
.L62     JMP ST1
.L7      CMP #key_cell  ; ascii 20 = space, toggle cell
         BNE L8
         JSR CELL
.L72     JMP ST1
.L8      CMP #key_start ; ascii 0D = return, start computing generations
         BNE L72
         LDA CSTAT
         BEQ L9
         LDA #&00
...
.L11     JSR OPTMV
         ...
         JSR DISPL
IF _atom
         JSR &FE71     ; read KEYBOARD
         BCS L11       ; loop back if no key pressed
ELSE
